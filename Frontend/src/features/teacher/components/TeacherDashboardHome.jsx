...
const StatCard = React.memo(
  ({ title, value, icon: Icon, change, trend, color = "red" }) => {
...
const QuickActionCard = React.memo(
  ({ title, description, icon: Icon, to, color = "red" }) => {
    const colorScheme = colorMaps[color] || colorMaps.red;
    ...
    return (
      <Link
        ...
        <div className="flex items-start">
          <div
            className={`flex-shrink-0 rounded-xl p-3 ${colorScheme.trendBg} ${colorScheme.trend} transition-colors duration-300 ${colorScheme.hoverBg}`}>
            <Icon className="h-6 w-6" />
          </div>
...
const TeacherDashboardHome = () => {
  ...
  const recentActivity = [
    {
      type: "students",
      text: "3 nuevos estudiantes",
      subtext: "Se inscribieron en tu curso de React",
      time: "Hace 2 horas",
      icon: Users,
      color: "green",
    },
    {
      type: "message",
      text: "Nuevo mensaje",
      subtext: "Tienes 5 mensajes sin leer",
      time: "Hace 4 horas",
      icon: MessageSquare,
      color: "blue",
    },
    ...
    <div className="bg-gray-50 min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
        <header className="mb-8 md:mb-12">
          ...
          <p className="text-base text-gray-500 mt-1 md:mt-2">
                ...
                <ul className="space-y-2">
                  {[
                    ...
                    },
                    {
                      ...
                      icon: Video,
                      label: "Guía de Producción",
                    },
                  ].map((link, index) => (
                    <li key={index}>
                      <Link
                        to={link.to}
                        className="flex items-center p-3 rounded-lg hover:bg-gray-50 text-gray-700 hover:text-red-600 transition-colors">
